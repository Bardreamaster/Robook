...
![20230222141551](https://robook-1313535466.cos.ap-guangzhou.myqcloud.com/20230222141551.png)  输出扭矩均为4Nm的两款无框电机——长筒状 or 扁平状 ？
...
> 其中 R 为转子半径；$ \mu_{0}$  为磁通量与磁场强度的转换系数；$ k_{s}$ 为线圈表面电流系数；$ L $为永磁体截面厚度（即外半径与内半径之差）；$ M_{0}$  为径向方向的磁化强度（magnet magnetization）在傅里叶展开的第一项； $\\omega$ 为轴向的长度； Gap  为永磁体与线圈之间的间隙长度。

因为我们在这里仅需要对比分析几何尺寸参数对输出扭矩带来的影响，所以我们把 $ \mu_{0}, k_{s}, M_{0} $的乘积写作常量，即$ A = \mu_{0}k_{s}M_{0} $，那么公式(1)可简化成：
...
![20230222141609](https://robook-1313535466.cos.ap-guangzhou.myqcloud.com/20230222141609.png)
...
![20230222141621](https://robook-1313535466.cos.ap-guangzhou.myqcloud.com/20230222141621.png)
...
\tau_{max} = V\cdot\tau_{v-max} = \pi\omega L(2R-L)\cdot\tau_{v-max} = 26.39Nm,
...
\tau_{max-actuator} = \tau_{max} \cdot G = 263.9Nm,
...
\tau_{max-density} = \frac{\tau_{max-actuator}}{2\cdot m_{rotor}} = 66.67Nm/kg,
